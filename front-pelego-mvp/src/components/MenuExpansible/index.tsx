...
export function MenuExpansible({ children, open, logo, onClick, footer }: MenuExpansibleProps) {
  return (
    <aside
      ...
      <div className="flex flex-col h-full">
        ...
        {footer && <div className="border-t border-border/50 p-3">{footer}</div>}
      </div>
    </aside>
  );
}
  ...
  if (!withSubItems) {
    return (
      <Link
        ...
        <div
          className={`${active ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-accent-foreground'}`}>
          {icon}
        </div>
  ...
  return (
    <div className="my-0.5">
      <button
        ...
        <div
          className={`${active ? 'text-primary' : 'text-muted-foreground group-hover:text-accent-foreground'}`}>
          {icon}
        </div>
      ...
      {open && !forceClosed && (
        <div className="flex flex-col pl-8 pr-2 py-1 gap-0.5 animate-fade-in">{children}</div>
      )}
    </div>
  );
}

export function MenuExpansibleContent({ children }: { children: React.ReactNode }) {
  return <div className="flex flex-col w-full px-3 gap-1">{children}</div>;
}

export function MenuExpansibleLogo({ children }: { children: React.ReactNode }) {
  return children;
}

export function MenuExpansibleContentHeader({ children }: { children: React.ReactNode }) {
