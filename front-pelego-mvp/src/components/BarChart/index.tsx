  ...
  return (
    <Card className="bg-[hsl(var(--card))] text-[hsl(var(--foreground))]  max-h-[500px] min-w-[400px]">
      <CardHeader>
        ...
        <CardDescription className="text-[hsl(var(--muted-foreground))] px-2">{description}</CardDescription>
      </CardHeader>
      <CardContent>
        <ChartContainer config={chartConfig}>
          ...
          >
            <CartesianGrid horizontal={false} 
              vertical={true}stroke="#27272a" strokeDasharray="none" /> {/* Linhas horizontais */}
            <YAxis
              dataKey="name"
              type="category"
              tickLine={false}
            ...
            <Bar
              ...
              layout="vertical"
              radius={5}
              fill={({ payload }) => payload.fill} // Adiciona a cor especificada no chartData
            >
              <LabelList 
                dataKey="value" 
                position="right" 
