...
const Badge: React.FC<BadgeProps> = ({ playerData, children }) => {
  ...
  return (
    <div
      className={`${getBackgroundImage()} ${getShadow()} h-[500px] w-[450px] bg-contain bg-center bg-no-repeat transition-smooth hover:scale-105`}>
      {children}
    </div>
  );
};
...
export const TeamOfTheMonthCardLarge: React.FC<{ playerData: Player; showOverall: boolean }> = ({
  playerData,
}) => {
  const borderColor =
