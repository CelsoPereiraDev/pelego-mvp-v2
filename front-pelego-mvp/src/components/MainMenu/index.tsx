...
export default function MainMenu() {
  ...
  const years = Array.from(
    new Set(weeks?.map(week => new Date(week.date).getFullYear()))
  );
    ...
    <MenuExpansible open={open} onClick={() => setOpen(!open)} footer={
      ...
      <MenuExpansibleContent>
        ...
        <MenuExpansibleItem label="Status" icon={<EmojiEventsIcon />} withSubItems>
          <MenuExpansibleItem label="Resumo" icon={<BarChartIcon />} withSubItems active={pathname.includes('stat-resume')}>
            {years.map(year => (
              <MenuExpansibleItem 
                key={year} 
          ...
          <MenuExpansibleItem label="Defensores" icon={<LocalPoliceIcon />} withSubItems active={pathname.includes('best-defender')}>
            {years.map(year => (
              <MenuExpansibleItem 
                key={year} 
          ...
          <MenuExpansibleItem label="Artilheiros" icon={<SportsSoccerIcon />} withSubItems active={pathname.includes('top-scorer')}>
            {years.map(year => (
              <MenuExpansibleItem 
                key={year} 
          ...
          <MenuExpansibleItem label="Assistentes" icon={<AutoAwesomeIcon />} withSubItems active={pathname.includes('top-assists')}>
            {years.map(year => (
              <MenuExpansibleItem 
                key={year} 
          ...
          <MenuExpansibleItem label="Pontuação" icon={<TableChartOutlinedIcon />} withSubItems active={pathname.includes('points')}>
            {years.map(year => (
              <MenuExpansibleItem 
                key={year} 
                href={`/points/${year}`} 
        ...
        <MenuExpansibleItem label="Presença" icon={<CheckCircleOutlineIcon />} withSubItems active={pathname.includes('appear')}>
          {years.map(year => (
            <MenuExpansibleItem 
              key={year} 
              href={`/appear/${year}`} 
