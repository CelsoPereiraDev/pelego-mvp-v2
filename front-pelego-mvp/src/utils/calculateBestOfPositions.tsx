...
export const calculateBestOfEachPosition = (weeks: WeekResponse[]): CalculateBestOfEachPositionProps => {
  ...
  eligiblePlayers.forEach((playerSummary) => {
    ...
    const assists = allPlayersAssistCount.find(assist => assist.name === playerSummary.playerName)?.assists || 0;
    const goalsAgainst = playerSummary.averageGoalsConceded || 0;
    const points = allPlayersPoint.find(stats => stats.name === playerSummary.playerName)?.points || 0;
    const championships = allPlayersChampionCount[playerSummary.playerName] || 0;

    let totalPoints = 0;

    if (playerSummary.position === 'ATK') {
      totalPoints = (goals * 0.8) + (assists * 0.3) + (championships * 2) + ((1 / (goalsAgainst + 1)) * 6) + (points * 0.1);
